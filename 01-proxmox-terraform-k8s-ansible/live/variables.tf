...
variable "template_vm_id" {
  description = "ID шаблона VM для клонирования"
  type        = number
  default     = 9000

  validation {
    condition     = var.template_vm_id > 0 && var.template_vm_id < 100000
    error_message = "template_vm_id должен быть положительным числом меньше 100000"
  }
}
...
variable "ansible_version" {
  description = "Версия Ansible для установки на control VM"
  type        = string
